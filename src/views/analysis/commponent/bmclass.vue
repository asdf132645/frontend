<template>
  <div class="bmclass">
    <h3 class="titleText">
      <span class="greenColor">BM</span> <span class="greenColor">C</span>lassification
    </h3>
    <div>
      <template v-for="(classList, outerIndex) in dspBmClassList" :key="outerIndex">
        <template v-for="(category, innerIndex) in classList" :key="innerIndex">
          <div class="categories" v-if="shouldRenderCategory(category)">
            <ul class="categoryNm">
              ...
              <li v-if="innerIndex === dspBmClassList.length && dspBmClassList.length !== 1">
                total
              </li>
            </ul>
            <ul class="classNm">
...
import {WbcInfo, basicBmClassList, BmInfo} from "@/store/modules/analysis/wbcclassification";
...
const dspBmClassList = ref<any>([]);
const dspBfClassList = ref<BmInfo[]>([]);
const nonRbcClassList = ref<BmInfo[]>([]);
...
const updateDataArray = async (newSlotInfo: BmInfo[]) => {
  ...
  if (Array.isArray(slotArray.wbcInfo)) {
    ...
    dspBmClassList.value = [basicBmClassList];
    dspBfClassList.value = dspBmClassList.value.flat();
  }
...
const updatePercentages = async () => {
  const percent = dspBmClassList.value.map((classList: any) => {
